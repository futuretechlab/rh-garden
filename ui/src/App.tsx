...
function FrontierCockpit({data, periods}: {data: FrontierData; periods: BusyPeriod[]}) {
  ...
  const finiteHardest = periods.length ? [...periods].sort((a,b) =>
    a.envelope_reserve_remaining - b.envelope_reserve_remaining)[0] : null
  return <section className="screen">
    ...
    {finiteHardest && <div className="frontier-target panel"><div><small>FINITE EVENT-COST AUDIT</small><strong>{finiteHardest.start_event} -&gt; {finiteHardest.recovery_before_event}</strong><span>reserve after sample-envelope costs {format(finiteHardest.envelope_reserve_remaining)}</span></div><div><small>LEANCHECKED FINITE VERIFIER</small><code>SuzukiFinitePartitionCertificate.nonnegative</code><code>integral_serviceDecayEnvelope_eq_exactCellCost</code></div><div><small>NUMERICAL SAMPLE ENVELOPE</small><strong>{finiteHardest.envelope_reserve_remaining >= 0 ? 'FITS SCAN' : 'FAILS'}</strong><span>cost above exact table {format(finiteHardest.envelope_cost_gap)} / transformed excess gap {format(finiteHardest.envelope_max_excess_gap)}</span></div></div>}
    <MilestoneStrip />
  </section>
}
...
function SuzukiField({explorer, proofMode}: {explorer: ExplorerData; proofMode: boolean}) {
  ...
  return <section className="screen">
    ...
    {proofMode ? <ProofModeVeil title="Numerical field hidden"><p>Proof Mode preserves the exact representation and certified intervals, while hiding sampled values.</p><div className="certified-range"><b>[0, log 37]</b><span>nonnegative; strict positivity proved on (0, log 3] · LeanChecked</span></div></ProofModeVeil> : <>
      <div className="chart-panel panel"><Chart series={[{name: `Ψ_${omega}(t)`, color: '#f2bc57', points}]} zeroLine certified={omega === 0 ? [0, Math.log(3)] : undefined}/><div className="chart-label left">Ψω(t)</div><div className="chart-label bottom">t →</div></div>
...
function BusyInspector({period}: {period: BusyPeriod | null}) {
  ...
  return <aside className="busy-inspector panel">
    ...
    <Metric label="finite exact-service cost" value={format(period.finite_event_exact_cost)}/>
    <Metric label="finite linear-service cost" value={format(period.finite_event_linear_cost)}/>
    <Metric label="finite cost residual" value={format(period.finite_event_cost_residual)}/>
    <Metric label="reserve after finite costs" value={format(period.finite_event_reserve_remaining)}/>
    <Metric label="jump-aware sample gap" value={format(period.jump_aware_max_gap)}/>
    <Metric label="sample-envelope cost" value={format(period.envelope_cost)}/>
    <Metric label="sample-envelope cost gap" value={format(period.envelope_cost_gap)}/>
    <Metric label="reserve after sample envelope" value={format(period.envelope_reserve_remaining)}/>
    <Metric label="event-log envelope cost" value={format(period.event_log_cost)}/>
    <Metric label="prime-power surcharge J" value={format(period.prime_power_surcharge)}/>
    <Metric label="real recovery square (not event label)" value={format(period.recovery_square)}/>
    <Metric label="surcharge identity residual" value={format(period.surcharge_identity_residual)}/>
    <Metric label="anchored pinned-prefix cost" value={format(period.pinned_anchored_cost)}/>
    <Metric label="local-width envelope cost" value={format(period.local_width_cost)}/>
    <Metric label="first local-width budget failure" value={period.local_width_first_failing_cell === null ? 'none' : String(period.local_width_first_failing_cell)}/>
    ...
    {profile.length > 1 && <div className="chebyshev-profile">
      ...
      <div className="mini-profile-chart"><Chart zeroLine series={[
        ...
        {name: 'sample event bound', color: '#f5e3a0', points: profile.map(p => ({x:p.root,y:p.event_excess_upper}))},
        ...
        {name: 'finite cumulative cost', color: '#5fe0c1', points: profile.map(p => ({x:p.root,y:p.cumulative_exact_cost}))},
        {name: 'sample-envelope cumulative cost', color: '#b2f7e3', points: profile.map(p => ({x:p.root,y:p.cumulative_envelope_cost}))},
      ]}/></div>
      ...
      <p><b>Finite-event audit:</b> each outgoing cell uses the exact service cutoff and the actual <code>2/u</code> weight. The displayed sample bound rounds <code>R(q)</code> upward to 0.001 and remains NumericalEvidence until separately proved in Lean.</p>
...
function ArithmeticFrontier({periods, proofMode}: {periods: BusyPeriod[]; proofMode: boolean}) {
  ...
  return <section className="screen">
    ...
    <NumericalBanner hidden={proofMode}/>
    {proofMode ? <ProofModeVeil title="Numerical interval diagnostics hidden"><div className="formula-stack"><code>finite event excess bounds eᵢ</code><code>Σ exactServiceCellCost(rᵢ,rᵢ₊₁,z₀+eᵢ) ≤ reserve</code><code>SuzukiFiniteEventProfileCertificate.psiRoot_nonnegative</code></div><p>The 17/30 marker is literature context, not a LeanChecked implication.</p></ProofModeVeil> : <div className="arithmetic-layout">
...
function CertificateWorkbench({busy}: {busy: BusyPeriod[]}) {
  ...
  return <section className="screen">
    ...
    <div className="certificate-grid">
      ...
      <article className="certificate formal"><TrustBadge trust="LeanChecked"/><h2>Initial interval</h2><code>suzukiInitialNonnegative_proved</code><p>The nine-term rational polynomial certificate proves Ψ(t) ≥ 0 on [0, log 2], with strict positivity for t &gt; 0.</p><div className="certificate-margin">INITIAL GAP <strong>CLOSED</strong></div></article>
      ...
      <article className="certificate formal"><TrustBadge trust="LeanChecked"/><h2>Finite event verifier</h2><code>SuzukiFiniteEventProfileCertificate.psiRoot_nonnegative</code><p>A complete finite prime-power chain, finite event bounds, and one exact total-cost inequality replace the continuum profile inputs.</p><div className="certificate-margin">FINITE GLUING <strong>READY</strong></div></article>
      <article className="certificate formal"><TrustBadge trust="LeanChecked"/><h2>Events 3, 4, 5</h2><code>suzukiFinitePartitionCertificate_three_five</code><p>The reserve 43/1000 pays cell-cost bounds 34/1000 + 9/1000. The prime-square arrival at 4 is included, and every point of [√3, √5] is nonnegative.</p><div className="certificate-margin">FINITE CERTIFICATE <strong>PROVED</strong></div></article>
    ...
    <article className="certificate formal"><TrustBadge trust="LeanChecked"/><h2>Finite prefix and m=31 obstruction</h2><code>suzuki_eventLog_obstruction_thirtyOne</code><p>Fresh cell reserves prove [0, log 37]. At the real recovery after 32, the true reserve is positive but the prime-power surcharge exceeds it. This refutes the unrestricted event-log budget, not Suzuki positivity. No universal threshold at 37 is proved.</p></article>
    <div className="tail-strip"><span>FINITE CERTIFIED COVERAGE</span><b>[0, log 37]</b><i /><span>UNCERTIFIED TAIL</span><b>(log 37, ∞)</b></div>
  </section>
}
...
function MilestoneStrip() { return <div className="milestones"><div><span>01</span><b>Shifted criterion</b><small>LeanChecked</small></div><div><span>02</span><b>Prime-cell convexity</b><small>LeanChecked</small></div><div><span>03</span><b>Initial interval</b><small>LeanChecked</small></div><div><span>04</span><b>Finite event verifier</b><small>LeanChecked</small></div><div className="open"><span>05</span><b>Universal event bounds</b><small>OPEN</small></div></div> }
