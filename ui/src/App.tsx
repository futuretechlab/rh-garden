...
function FrontierCockpit({data, periods}: {data: FrontierData; periods: BusyPeriod[]}) {
  ...
  const finiteHardest = periods.length ? [...periods].sort((a,b) =>
    a.finite_event_reserve_remaining - b.finite_event_reserve_remaining)[0] : null
  return <section className="screen">
    ...
    {finiteHardest && <div className="frontier-target panel"><div><small>FINITE EVENT-COST AUDIT</small><strong>{finiteHardest.start_event} -&gt; {finiteHardest.recovery_before_event}</strong><span>remaining sampled reserve {format(finiteHardest.finite_event_reserve_remaining)}</span></div><div><small>LEANCHECKED FINITE VERIFIER</small><code>SuzukiFiniteEventProfileCertificate.psiRoot_nonnegative</code><code>integral_serviceDecayEnvelope_eq_exactCellCost</code></div><div><small>NUMERICAL EVENT TABLE</small><strong>{finiteHardest.finite_event_reserve_remaining >= 0 ? 'FITS SCAN' : 'FAILS'}</strong><span>exact-cost residual {format(finiteHardest.finite_event_cost_residual)} / jump-aware gap {format(finiteHardest.jump_aware_max_gap)}</span></div></div>}
    <MilestoneStrip />
  </section>
}
...
function SuzukiField({explorer, proofMode}: {explorer: ExplorerData; proofMode: boolean}) {
  ...
  return <section className="screen">
    ...
    {proofMode ? <ProofModeVeil title="Numerical field hidden"><p>Proof Mode preserves the exact representation and certified intervals, while hiding sampled values.</p><div className="certified-range"><b>[0, log 3]</b><span>nonnegative, and positive away from 0 · LeanChecked</span></div></ProofModeVeil> : <>
...
function CertificateWorkbench({busy}: {busy: BusyPeriod[]}) {
  ...
  return <section className="screen">
    ...
    <div className="tail-strip"><span>FINITE CERTIFIED COVERAGE</span><b>[0, log 3]</b><i /><span>UNCERTIFIED TAIL</span><b>(log 3, ∞)</b></div>
  </section>
}
